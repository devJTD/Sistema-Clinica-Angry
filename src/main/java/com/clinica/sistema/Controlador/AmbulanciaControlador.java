...
public class AmbulanciaControlador {
    ...
    // Método auxiliar para obtener el objeto Paciente del usuario actualmente logueado y establecer el MDC.
    private Paciente getPacienteLogueado() {
        ...
        if (authentication == null || !authentication.isAuthenticated() || authentication.getPrincipal() instanceof String) {
            ...
            // No limpiar MDC aquí, ya que este método es llamado por otros que pueden haberlo establecido.
            // La limpieza se hará en el 'finally' de cada método de controlador.
            return null;
        }
        ...
        if (pacienteOpt.isPresent()) {
            ...
            logger.warn("No se encontró paciente en la base de datos para el correo: {}.", correoUsuario);
            // No limpiar MDC aquí, dejar que el 'finally' del controlador lo haga.
        }
    ...
    public String mostrarPaginaAmbulancia(Model model) {
        // Primero, intentar obtener el paciente logueado, lo que establecerá el MDC si el paciente es encontrado.
        Paciente pacienteLogueado = getPacienteLogueado();

        try {
            // El primer log ahora usa el MDC, si está disponible.
            if (MDC.get(MDC_USER_FULL_NAME) != null) {
                logger.info("El paciente {} (ID: {}, DNI: {}) ha accedido a la página de solicitud de ambulancia.",
                            MDC.get(MDC_USER_FULL_NAME), MDC.get(MDC_USER_ID), MDC.get(MDC_USER_DNI));
            } else {
                logger.info("Un usuario no autenticado ha accedido a la página de solicitud de ambulancia.");
            }
            ...
            if (pacienteLogueado == null || pacienteLogueado.getId() == null) {
                logger.warn("Redirigiendo a login: Usuario no logueado o sin ID intentó acceder a la página de ambulancia.");
            ...
            clearPacienteMDCContext(); // Limpiar MDC al finalizar el método del controlador
        }
    ...
    public ResponseEntity<?> guardarNuevaDireccion(@RequestBody Map<String, String> payload) {
        Paciente pacienteLogueado = getPacienteLogueado(); // Esto establecerá el MDC si el paciente es encontrado

        try {
            // Si no está logueado, retorna un error de no autorizado.
            if (pacienteLogueado == null || pacienteLogueado.getId() == null) {
                logger.warn("Intento de guardar nueva dirección por usuario no logueado o sin ID de paciente. Retornando no autorizado.");
            ...
            logger.info("El paciente {} (ID: {}, DNI: {}) está intentando guardar una nueva dirección. Datos recibidos: [Dirección Completa: {}]",
            ...
            if (direccionCompleta == null || direccionCompleta.trim().isEmpty()) {
                logger.warn("Intento fallido de guardar dirección: La dirección no puede estar vacía para el paciente {} (ID: {}, DNI: {}).",
                            MDC.get(MDC_USER_FULL_NAME), MDC.get(MDC_USER_ID), MDC.get(MDC_USER_DNI));
            ...
            logger.info("El paciente {} (ID: {}, DNI: {}) ha guardado una nueva dirección con ID: {}. Dirección: {}",
            ...
            logger.error("Error interno al guardar la nueva dirección para el paciente {} (ID: {}, DNI: {}): {}",
                        MDC.get(MDC_USER_FULL_NAME), MDC.get(MDC_USER_ID), MDC.get(MDC_USER_DNI), e.getMessage(), e);
    ...
    public ResponseEntity<?> solicitarAmbulancia(@RequestBody Map<String, Long> payload) {
        ...
        try {
            ...
            logger.info("El paciente {} (ID: {}, DNI: {}) está intentando solicitar una ambulancia. Datos recibidos: [ID Dirección Seleccionada: {}]",
            ...
            if (direccionId == null) {
                logger.warn("Intento fallido de solicitar ambulancia: Debe proporcionar una ID de dirección para el paciente {} (ID: {}, DNI: {}).",
                            MDC.get(MDC_USER_FULL_NAME), MDC.get(MDC_USER_ID), MDC.get(MDC_USER_DNI));
            ...
            if (direccionOpt.isEmpty()) {
                logger.warn("Intento fallido de solicitar ambulancia: La dirección con ID: {} no es válida o no pertenece al paciente {} (ID: {}, DNI: {}).",
            ...
            logger.info("El paciente {} (ID: {}, DNI: {}) ha procesado exitosamente la solicitud de ambulancia con dirección: {} (ID: {}).",
