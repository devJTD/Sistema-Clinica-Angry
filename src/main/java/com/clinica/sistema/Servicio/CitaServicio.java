...
public class CitaServicio {
        ...
        public Cita crearCita(String fechaStr, String horaStr, Long idMedico, Long idPaciente) {
                ...
                if (idMedico == null || idMedico <= 0) {
                        logger.warn("[CitaServicio] - Validación fallida: El ID del médico es nulo o inválido ({}).",
                                        idMedico);
                        throw new IllegalArgumentException("El ID del médico no puede ser nulo o negativo.");
                }
                if (idPaciente == null || idPaciente <= 0) {
                        logger.warn("[CitaServicio] - Validación fallida: El ID del paciente es nulo o inválido ({}).",
                                        idPaciente);
                ...
                Paciente paciente = pacienteRepositorio.findById(idPaciente)
                                .orElseThrow(() -> {
                                        logger.error("[CitaServicio] - Error: Paciente con ID {} no encontrado.",
                                                        idPaciente);
                ...
                Medico medico = medicoRepositorio.findById(idMedico)
                                .orElseThrow(() -> {
                                        logger.error("[CitaServicio] - Error: Médico con ID {} no encontrado.",
                                                        idMedico);
                                        return new IllegalArgumentException(
                ...
                logger.info("[CitaServicio] - Buscando Horario disponible para fecha: {}, hora: {}, y médico ID: {}",
                                fechaCita,
                                horaCita, idMedico);
                Optional<Horario> horarioOptional = horarioRepositorio.findByFechaAndHoraAndMedico(fechaCita, horaCita,
                                medico);
                ...
                logger.info("[CitaServicio] - Horario encontrado: ID {}, Fecha: {}, Hora: {}, Disponible: {}",
                                horario.getId(),
                ...
                logger.debug(
                                ...
                                nuevaCita.getFecha(), nuevaCita.getHora(), nuevaCita.getEstado(),
                                nuevaCita.getPaciente().getId(),
                ...
                try {
                        ...
                        nuevaNotificacion.setFechaEnvio(LocalDate.now()); // Mantenido como LocalDate.now() según tu
                                                                          // indicación
        ...
        public boolean cancelarCita(Long idCita) {
                ...
                if (citaOptional.isPresent()) {
                        ...
                        if (horarioOptional.isPresent()) {
                                ...
                                logger.debug("[CitaServicio] - Horario ID {} encontrado para re-disponibilidad.",
                                                horario.getId());
                                horario.setDisponible(true);
                                horarioRepositorio.save(horario);
                                logger.info(
                                                "[CitaServicio] - Horario {}-{}-{} del médico {} marcado como disponible tras cancelación de cita {}.",
                                                horario.getFecha(), horario.getHora(), horario.getMedico().getId(),
                                                idCita);
                        } else {
                                logger.warn(
                        ...
                        logger.warn("[CitaServicio] - Intento de cancelar cita con ID {} fallido: Cita no encontrada.",
                                        idCita);
                        return false;
                }
        }
        ...
        public List<Medico> obtenerMedicosPorEspecialidad(Long idEspecialidad) {
                ...
                Especialidad especialidad = especialidadRepositorio.findById(idEspecialidad)
                                .orElseThrow(() -> {
                                        logger.error("[CitaServicio] - Error: Especialidad con ID {} no encontrada.",
                                                        idEspecialidad);
                                        return new IllegalArgumentException(
        ...
        public List<Horario> obtenerHorariosDisponiblesPorMedicoYFecha(Long idMedico, LocalDate fecha) {
                logger.info("[CitaServicio] - Buscando horarios disponibles para Médico ID: {} y Fecha: {}.", idMedico,
                                fecha);
                Medico medico = medicoRepositorio.findById(idMedico)
                                .orElseThrow(() -> {
                                        logger.error("[CitaServicio] - Error: Médico con ID {} no encontrado.",
